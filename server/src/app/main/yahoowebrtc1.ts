...
import { IBApi, EventName, ErrorCode, Contract, Stock } from '@stoqey/ib';
  ...
  private async collectInputs() {
    ...
    if (this.initialinvestment > 0) {
      ...
      this.symbols[this.symbol] = {
        ...
        tracking: false,
        contract: cont,
        buyingPower: this.initialinvestment * this.marginMultiplier
      } as Symbol;
...
/*    this.ib.on("accountSummary", (reqId, account, tag, value) => {
      if (tag === 'AvailableFunds') {
        this.buyingPower = Math.min(parseFloat(value), this.initialinvestment * this.marginMultiplier);
      }
    });*/
  }
  ...
  private setupSequentialHandlers(): void {
    const allEvents: { [K in keyof typeof EventName]?: (...args: any[]) => Promise<void> } = {
      tickPrice: async (...args: [number, number, number]) => this.onTickPrice(...args),
      orderStatus: async (...args: any[]) => this.onOrderStatus(...args as [number, string, number, number, number]),
      openOrder: async (...args: any[]) => this.onOpenOrder(...args as [number, any, any, string]),
      nextValidId: async (...args: any[]) => this.onNextValidId(...args as [number]),
      position: async (...args: any[]) => this.onPosition(...args as [any, any, any, any]),
      pnlSingle: async (...args: any[]) => {
        this.onPnL(...args as [number, number, number, number, number, number])
      }
    };
  ...
  private async onTickPrice(tickerId: number, field: number, price2: number): Promise<void> {
...
//      if (sym.tobuy || ((now - lastTime >= 60000)) && this.orders[symbol] && this.orders[symbol].action === 'BUY') {
  ...
  private async onOrderStatus(orderId, status, filled, remaining, avgFillPrice): Promise<void> {
    const order = Object.values(this.orders).find(o => o.orderId === orderId);
    if (order && orderId === order.orderId && status === 'Filled') {
...
//            await this.ib.cancelPnLSingle(contract.handler);
    ...
    } else if (order && orderId === order.orderId && status === 'Cancelled') {
      const contract = this.symbols[order.symbol]?.contract;
      const sym = this.symbols[order.symbol];
      if (order.action === 'SELL') {
        sym.tobuy = false;
  ...
  private async onOpenOrder(orderId, contract, order, orderState): Promise<void> {
  ...
  private async onPosition(account, contract, pos, avgCost?): Promise<void> {
    if (pos > 0) {
  ...
  private async onPnL(i, pos, dailyPnL, unrealizedPnL, realizedPnL, value): Promise<void> {
